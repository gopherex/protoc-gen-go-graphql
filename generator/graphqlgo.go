...
func (e *gqlEmitter) emit(outPkg, runtimeImport string) (string, error) {
	...
	body.WriteString("\t// Authorize, when set, runs before each operation delegates to its gRPC\n")
	body.WriteString("\t// method, receiving the gRPC procedure name and decoded request, to enforce\n")
	body.WriteString("\t// the same per-method authn+authz as the connect/gRPC interceptors. It\n")
	body.WriteString("\t// returns the (identity-enriched) context to delegate with.\n")
	body.WriteString("\tAuthorize func(ctx context.Context, procedure string, req interface{}) (context.Context, error)\n")
	body.WriteString("}\n\n")
	...
	// Third-party imports, sorted by path (gofmt order within the group). graphql
	// has no alias; runtime + pb + cross-package payload pkgs are aliased.
	...
	for _, p := range e.extraImps {
		...
		if !strings.Contains(bodyStr, alias+".") {
			continue // payload package not referenced
		}
		imps = append(imps, imp{alias, p})
	}
...
func (e *gqlEmitter) emitOpField(svc *protogen.Service, m *protogen.Method, op string) string {
	...
	// authz preamble: bind ctx to the (possibly identity-enriched) authorized
	// context before delegating, mirroring the connect/gRPC interceptors.
	authz := func(indent string) string {
		return fmt.Sprintf("%sctx := p.Context\n%sif srv.Authorize != nil {\n%s\tvar aerr error\n%s\tctx, aerr = srv.Authorize(ctx, %q, req)\n%s\tif aerr != nil { return nil, %s.GraphQLError(ctx, aerr) }\n%s}\n",
			indent, indent, indent, indent, procedure, indent, e.runtime, indent)
	}
	var b strings.Builder
	switch op {
	case "Query", "Mutation":
		...
		fmt.Fprintf(&b, "\t\t\treq := %s\n", reqExpr)
		b.WriteString(authz("\t\t\t"))
		fmt.Fprintf(&b, "\t\t\tresp, err := srv.%s.%s(ctx, req)\n", svc.GoName, m.GoName)
		fmt.Fprintf(&b, "\t\t\tif err != nil { return nil, %s.GraphQLError(ctx, err) }\n\t\t\treturn resp, nil\n\t\t}}", e.runtime)
	case "Subscription":
		fmt.Fprintf(&b, "&graphql.Field{Type: %s, ", objVar(gqlTypeName(m.Output)))
		if args != "" {
			fmt.Fprintf(&b, "Args: %s, ", args)
		...
		fmt.Fprintf(&b, "\t\t\t\treq := %s\n", reqExpr)
		b.WriteString(authz("\t\t\t\t"))
		fmt.Fprintf(&b, "\t\t\t\treturn %s.PumpServerStream[%s](ctx, func(ss *%s.StreamServer[%s]) error {\n", e.runtime, respGo, e.runtime, respGo)
